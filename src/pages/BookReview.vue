      ...
      <div v-else class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="flex flex-col items-center lg:items-start">
          ...
          <DefaultButton @click="$router.push({ path: `/books/${book?.title}`, query: { pageNumber: 1 } })"
            class="w-80 mt-4 text-2xl px-8 py-2">
            Читать
          </DefaultButton>
        </div>
      ...
      <div v-if="!loading && !error" class="bg-gray-200 p-6 rounded-2xl mt-8">
        <div class="flex flex-wrap gap-4 justify-center">
          <div @click="$router.push({ path: '/search', query: { genres: genre.name } })" v-for="genre in book?.genres"
            :key="genre.id" class="bg-[#A9A8FD] text-white font-lora text-xl px-6 py-2 rounded-full cursor-pointer">
            {{ genre.name }}
          </div>
        </div>
      </div>
...
const bookImageUrl = computed(() => {
  return book.value ? `http://localhost:8080/${book.value.image}` : '';
});

onMounted(async () => {
  try {
    const response = await axios.get(`http://localhost:8080/books/${route.params.bookTitle}`);
    if (!response.data) {
      throw Error('Такой книги нет mutherfucker'); // TODO Нужно добавить страницу ошибки
    }
    book.value = response.data;
  } catch (err) {
    error.value = (err as Error).message || 'Ошибка при загрузке данных';
  } finally {
    loading.value = false;
  }
});
